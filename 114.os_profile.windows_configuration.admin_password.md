...
**Classification:** Create phase → Assign to `local.sensitive_body` (sensitive field)
...
✅ **MATCH** - Predicted path matches the verified assignment chain from Go source code. The field is set at osProfile level as documented.
...
- ✅ Property in correct local (`local.sensitive_body`)
- ✅ ForceNew wrapped: `{ value = var.field }`
- ✅ ALL logic EXACTLY replicated from provider (validations, DiffSuppressFunc analysis)
- ✅ Validations IMPLEMENTED in migrate_variables.tf (length, complexity, disallowed values)
- ✅ TODO comment added to original field in variables.tf (same line)
- ✅ Hidden fields checked (none)
- ✅ Deferred work in following.md: None deferred from this task
- ✅ Deferred work from following.md: No work deferred TO this task
- ✅ Critical review (null, edge, idempotent, safe refs)
- ✅ Edge Case Analysis included (shared adminPassword key, error message inconsistency, complexity validation, version required)
- ✅ Proof created
- ✅ `track.md` to be updated to Pending for check
- ✅ Self-Review: Only admin_password implemented, no other windows_configuration fields added
...
**Expected Behavior:**
- When `windows_configuration` is null → version can be null (no password required)
- When `windows_configuration` is not null → `admin_password` is always set (Required field with `nullable = false`) → version must be non-null
...
**Verification:**
- Scenario 1: `os_profile = null` → condition: `true || ... = true` ✅ (version can be null)
- Scenario 2: `os_profile.windows_configuration = null` → condition: `false || true || ... = true` ✅ (version can be null)
- Scenario 3: `os_profile.windows_configuration` exists, version = null → condition: `false || false || false = false` → ERROR ✅ (enforces version requirement)
- Scenario 4: `os_profile.windows_configuration` exists, version = 1 → condition: `false || false || true = true` ✅ (allows non-null version)

### Final Verification Checklist

Before signing approval, performed mandatory final verification:

1. **Re-read all issues identified:** ✅ Only one issue found (version validation)
2. **For EACH issue:**
   - ✅ Code changes made to `migrate_variables.tf`
   - ✅ Re-read the modified file to confirm changes are present
   - ✅ Verified the changes match documented corrections
   - ✅ No related issues missed
3. **Cross-check bidirectional constraints:**
   - ✅ N/A - no bidirectional constraints for this field
4. **Verify all files mentioned in "Changed Files" section:**
   - ✅ `migrate_variables.tf` changes confirmed
   - ✅ No placeholder comments
   - ✅ No partial implementations
5. **Run mental test scenarios:**
   - ✅ Null handling verified (scenarios 1-4 above)
   - ✅ Empty string not possible (8-character minimum enforced)
   - ✅ Boundary conditions: 8-123 characters validated
   - ✅ Complexity validation correctly checks all 4 combinations of 3-of-4 character types
6. **Verify no new issues introduced:**
   - ✅ Fixes don't break existing code
   - ✅ No syntax errors
   - ✅ Proper HCL formatting

### Validation Results

✅ **ForceNew Logic:** Simple ForceNew with value tracking in `replace_triggers_external_values`
✅ **Stable Keys:** All keys in `replace_triggers_external_values` are stable
✅ **Phase Detection:** Field correctly placed in `local.sensitive_body` (Create phase, sensitive field)
✅ **Type Conversion:** String → String (direct mapping)
✅ **Null Handling:** Required field when `windows_configuration` exists (nullable = false), version validation now correctly conditional
✅ **Validations:** All provider validations implemented (length 8-123, complexity 3-of-4 character types, disallowed values list)
✅ **Sensitive Field Handling:** Independent ephemeral variable, version tracking, TODO comment in variables.tf, placed in `sensitive_body`
✅ **Deferred Work Completion:** No deferred work for this task
✅ **Deferred Work Recording:** No deferrals made by this task
✅ **Edge Cases:** Shared `adminPassword` key with Linux correctly handled via conditional version tracking logic
✅ **DiffSuppressFunc:** Correctly analyzed as no-op in normal operation (only handles import scenarios)
