...
**❌ VIOLATION FOUND:** AzAPI 2.0+ compatibility issue

**Line 53 in migrate_main.tf - BEFORE FIX:**
```hcl
existing_network_api_version = data.azapi_resource.existing.output != null ? try(data.azapi_resource.existing.output.properties.virtualMachineProfile.networkProfile.networkApiVersion, "") : ""
```

**Problem:** Used `data.azapi_resource.existing.output != null` instead of `.exists`

**Line 53 in migrate_main.tf - AFTER FIX:**
```hcl
...
**Reference:** Per `checker.md` lines 100-113, AzAPI 2.0+ requires using `.exists` property instead of checking `.output != null`
...
✅ **State Reading:** Uses `data "azapi_resource"` to read existing state for comparison (NOW FIXED: uses `.exists`)  
...
✅ **AzAPI 2.0+ Compliance:** NOW FIXED - uses `.exists` instead of `.output != null`
...
One AzAPI 2.0+ compatibility issue was identified and corrected. The implementation now correctly:
- Uses `.exists` property for existence checks (AzAPI 2.0+ requirement)
- Handles legacy mode (sku_name not set) suppression when transitioning from empty to "2020-11-01"
- Handles non-legacy mode (sku_name set) where all changes trigger updates
- Applies default matching provider schema
- Implements conditional presence based on sku_name or network_interface
