...
**Path Comparison:** ✅ Match - The `name` is used as part of the resource identifier, not in the request body. In AzAPI terms, this is part of the `azapi_header`, not `body`.
...
✅ The `name` field is deterministic and produces the same result on repeated applies. Since it's part of the resource identifier, changes trigger resource recreation, ensuring idempotency.

### Safe References

✅ Direct variable reference `var.orchestrated_virtual_machine_scale_set_name` is safe because the field is required and non-nullable.

## Checklist

- ✅ Property in correct local (azapi_header)
- ✅ ForceNew handled (inherent in azapi_header)
- ✅ Validations IMPLEMENTED in variables.tf (5 validation blocks added)
- ✅ Hidden fields checked (none)
- ✅ Critical review completed (null, edge, idempotent, safe refs)
- ✅ Edge Case Analysis included
- ✅ Proof created
- ✅ Self-Review: Only the `name` field implementation added (azapi_header creation is required for Task #1 per executor.md)
