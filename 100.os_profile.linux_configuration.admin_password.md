...
Field belongs in **Create Phase** → implement in `local.sensitive_body` (not `local.post_creation_updates`).
...
1. `input["admin_password"]` → `osProfile.AdminPassword` (in expand function)
2. `osProfile` → `virtualMachineProfile.OsProfile`
3. `virtualMachineProfile` → `props.Properties.VirtualMachineProfile`
...
- ✅ Independent ephemeral variable in `migrate_variables.tf`
- ✅ Version variable with validation
- ✅ TODO comment on original field in `variables.tf`
- ✅ Field value placed in `sensitive_body` (not `body`)
...
- When suppress condition is true → No replacement even if value changes
- When suppress condition is false → Normal ForceNew behavior applies
...
- Requires sum ≥ 3 (at least 3 of 4 conditions met)
...
- ✅ Field marked `ephemeral = true` (required for sensitive fields in Terraform 1.10+)
- ✅ Set in `sensitive_body` (not `body`)
- ✅ Version tracked with constant `"null"` to prevent automatic updates
- ✅ Manual updates controlled via `post_creation_updates` trigger
...
- `null` → Field not set in API call (valid)
- `""` → Field set to empty string (invalid, fails validation)
...
- ✅ Property in correct local (`sensitive_body` for initial, `post_creation_updates` for updates)
- ✅ DiffSuppressFunc logic replicated exactly
- ✅ Validations implemented (length, complexity, disallowed values)
- ✅ Independent ephemeral variable created for nested block sensitive field
- ✅ Version variable with validation created
...
- ✅ Critical review completed (null semantics, special values, security)
- ✅ Edge Case Analysis section included
- ✅ Proof created with all required sections
- ✅ track.md will be updated to "Pending for check"
- ✅ Self-Review: Only admin_password implementation added, no other fields included

## Self-Review Confirmation

**Scope Check**:
- ✅ Implemented ONLY `os_profile.linux_configuration.admin_password`
- ✅ Did NOT add any other fields from linux_configuration (computer_name_prefix, disable_password_authentication, etc.)
- ✅ Did NOT add fields from other tasks
- ✅ Did NOT add hidden fields (none for this task)

**Implementation Completeness**:
- ✅ Independent ephemeral variable with all validations
- ✅ Version variable with dependency validation
- ✅ TODO comment on original field
- ✅ DiffSuppressFunc logic with existing state reading
- ✅ Field in sensitive_body with conditional check
- ✅ Constant version tracking in sensitive_body_version
- ✅ Field in post_creation_updates body
- ✅ Trigger in replace_triggers_external_values

**No Deviations**:
- ✅ Exact replication of DiffSuppressFunc logic
- ✅ Exact replication of all validations
- ✅ No "simpler" or "more conservative" approaches taken
- ✅ Proof document contains no forbidden phrases justifying deviations
...
✅ Version variable with dependency validation  
✅ TODO comment on original field  
✅ Field in `sensitive_body` (not `body`)  
✅ `data.azapi_resource.existing` block (still needed by other fields)
