# Task #136 - plan.product - COMPLETED

## Summary

Implemented the Required argument `plan.product` (string) within the existing `plan` block skeleton. The field is a simple string with no validations, directly mapped to the Azure API `product` field. The field is ForceNew and tracked in `replace_triggers_external_values`.
...
The `expandPlanVMSS` function creates a `virtualmachinescalesets.Plan` object with `Name`, `Product`, and `Publisher` fields. The `Product` field is set from `raw["product"].(string)`.
...
**Verified Path:** `body.plan.product`

### Path Comparison

- **Predicted:** `body.plan.product` ✅
- **Verified:** `body.plan.product` ✅
- **Match:** YES
...
- Located at `body.plan.product` (root level of body, NOT under properties)
...
|------------------------|----------------------|-------|
| product | product | Direct mapping, no conversion needed |

## Special Handling

### ForceNew

The field is marked as `ForceNew: true` in the provider schema. This means any change to `plan.product` will trigger resource replacement.
...
**Key Stability:** The key `plan_product` is always present in the map. When `plan` block is null, the value is an empty string. This ensures the key doesn't appear/disappear across applies.
...
Checked `following.md` for any work deferred to Task #136: None found.
...
- **Empty string:** Allowed by schema (TypeString with no validations). Empty string is a valid product value.
...
- Parent conditional check `var.orchestrated_virtual_machine_scale_set_plan != null` ensures plan object exists before accessing `.product`
- Field is Required in schema, so `.product` will always be present when plan object exists
- No nested null checks needed

### ForceNew Stability

- Used stable key wrapping: `plan_product = { value = ... }`
...
- ✅ Property in correct local (`local.body.plan.product`)
- ✅ ForceNew tracked in `replace_triggers_external_values` with stable key
- ✅ Assignment path verified (`body.plan.product` matches provider behavior)
...
- ✅ Only Task #136 scope implemented (no other fields added)
...
**Date:** 2025-12-11
**Task:** #136 - plan.product

### Validation Results

✅ **ForceNew Logic:** Simple ForceNew (schema `ForceNew: true`) - correctly wrapped with stable key `plan_product = { value = ... }`
✅ **Stable Keys:** Key `plan_product` is always present in `replace_triggers_external_values` (empty string when plan is null)
...
✅ **Null Handling:** Parent conditional check ensures safe access to `.product` field
✅ **Validations:** None required - provider schema has no validations for this field
✅ **Assignment Path:** Correct path `body.plan.product` (root-level in body, NOT under properties)
✅ **Sensitive/WriteOnly:** Correctly placed in `local.body` (not Sensitive field)
✅ **Shared Path Merge:** No duplicate `plan` keys - appears only once at lines 647-653
✅ **Deferred Work Completion:** No deferred work for this task in following.md
✅ **Deferred Work Recording:** No deferrals made by this task
✅ **Edge Cases:** Empty string allowed, no length restrictions, idempotent behavior
✅ **Scope Compliance:** Only Task #136 content added (no hidden fields, no other tasks' fields)

### Compliance Statement

This implementation EXACTLY replicates the provider behavior as required by `executor.md`. The field is:
- Correctly marked as ForceNew with stable key wrapping
- Properly placed in Create phase (`local.body`)
- Assigned to correct API path (`body.plan.product`)
- Required field with no validations (matches provider schema)
- Safe null handling through parent conditional check

No deviations, simplifications, or "safer alternatives" were found. The implementation matches the provider's behavior line-by-line.
